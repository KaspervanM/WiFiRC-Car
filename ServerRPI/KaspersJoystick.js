...
$(document).ready(
  setTimeout(function () {
    ...
    joystick.style.width = ((33 * joystick_box.clientWidth) / 100)
      .toString()
      .concat("px");
    joystick.style.height = ((33 * joystick_box.clientHeight) / 100)
      .toString()
      .concat("px");
    ...
    function dragElement(elmnt) {
      ...
      function elementDrag(e) {
        ...
        if (
          Math.hypot(getPos().x, getPos().y) >
          joystick_box.clientHeight / 2
        ) {
          elmnt.style.top = elmnt.offsetTop + pos2 + "px";
          elmnt.style.left = elmnt.offsetLeft + pos1 + "px";
        }
...
function getPos(parentonly = -1) {
  ...
  joystick_box_pos_middle.y =
    joystick_box_pos.top + (joystick_box_pos.bottom - joystick_box_pos.top) / 2;
  joystick_box_pos_middle.x =
    joystick_box_pos.left +
    (joystick_box_pos.right - joystick_box_pos.left) / 2;

  if (parentonly > 0) return joystick_box_pos_middle;

  var joystick_pos = joystick.getBoundingClientRect();
  var joystick_pos_middle = {};
  joystick_pos_middle.y =
    joystick_pos.top + (joystick_pos.bottom - joystick_pos.top) / 2;
  joystick_pos_middle.x =
    joystick_pos.left + (joystick_pos.right - joystick_pos.left) / 2;

  var joystick_pos_middle_relative = relativate(
    joystick_box_pos_middle,
    joystick_pos_middle
  );
